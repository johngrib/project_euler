...
object Problem022 {
  ...
  def main(args: Array[String]) {
    ...
    def calc(count: Int, ls: List[Int], result: Int): Int =
      if (ls.length < 1) result
      else calc(count + 1, ls.tail, result + ls.head * count) // 재귀

    val lines = scala.io.Source.fromFile("data/names.txt").mkString.replaceAll("\"", "")
      .split(",").toList.sortWith(_ < _).map(_.toList.map(_.toInt - 64).sum)

    println(calc(1, lines, 0))
  }
}
